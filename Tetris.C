...
void again();void print();void remove1();void frame();
int score=0,spd,q,tt,a=0,i,j,l[6]={300,90,315,105,301,91},p,turn,m,n;
char play='y';
...
outtextxy(140,3,"TETRIS CLASSIC");
 setcolor( YELLOW );
settextstyle (DEFAULT_FONT , HORIZ_DIR ,2 );
outtextxy(19,95,"ENTER YOUR CHOICE");
 setcolor(CYAN );
settextstyle (DEFAULT_FONT , HORIZ_DIR ,2 );
outtextxy(25,140," 1.BASIC LEVEL");
setcolor(LIGHTGREEN );
settextstyle (DEFAULT_FONT , HORIZ_DIR ,2 );
outtextxy(25,160," 2.MIDDLE LEVEL");
setcolor(LIGHTBLUE );
settextstyle (DEFAULT_FONT , HORIZ_DIR ,2 );
outtextxy(25,200," 3.ADVANCED LEVEL");
setcolor(MAGENTA );
settextstyle (DEFAULT_FONT , HORIZ_DIR ,2 );
outtextxy(25,230," 4.EXPERTS LEVEL");
setcolor(WHITE);
settextstyle (DEFAULT_FONT , HORIZ_DIR ,2 );
outtextxy(25,260," 5.PROFESSIONAL LEVEL");
   y=getch();
   clrscr();
   cleardevice();
 ...
 check3()
{
 ...
 for(q=1;q<12;)
   if(arr[tt][q]==1) { q++; if(q==12) {score+=100;del();}}  else break;
